...
# ╔═╡ be2afcd0-0e26-11f0-2806-3f69fb937ce0
begin
	using Graphs
	using JSON3
	using DataFrames, JSON
	using CairoMakie, AlgebraOfGraphics, GraphMakie, NetworkLayout
	using Statistics
	using Distances
	using DelimitedFiles
end

# ╔═╡ c6776436-aa7b-4b8f-928b-636cdf5a7c63
fps = BitMatrix(readdlm("../data/inchi.fp_ecfp4.txt", Bool)')

# ╔═╡ a06d9192-ee28-49fe-8c7f-06e3922f890b
D = Distances.pairwise(Jaccard(), fps, fps)

# ╔═╡ 28dab927-9a17-43e6-b037-b549b5487435
df = DataFrame([merge(v, Dict("id" => k)) for (k,v) in JSON.parse(open("../data/MA_values.json", "r"))])

# ╔═╡ b966d00e-7bbb-4bf0-b0f0-914c2b6abe2e
heatmap(D[sortperm(df, :MA),sortperm(df, :MA)])

# ╔═╡ 25e8c4d0-de9b-4d40-bc9b-dcd7d0c0570a
sortperm(df, :MA)

# ╔═╡ 729571a3-9da6-44f8-8f03-dbc30e422131
mst = Graph(D .< 0.6)

# ╔═╡ 62e1f9e7-ca87-4d53-84c7-42fdf2192947
graphplot(mst, layout=Stress(), node_color=df[!,:MA])
...
manifest_format = "2.0"
project_hash = "6abc202719c87348a0d18eae6fadc6e4847ca4c7"
