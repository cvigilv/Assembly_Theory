...
# ╔═╡ 76915146-0aed-11f0-00a2-33b34f774e54
begin
	using MolecularGraph
	using DataFrames, CSV, JSON
	using CairoMakie, AlgebraOfGraphics, GraphMakie
	using HTTP
	using Statistics
	using Graphs, NetworkLayout
end

# ╔═╡ ece3963f-eebb-407d-b7f1-66e604e174fe
using JSON3

# ╔═╡ f72fb224-0a04-4596-ac8b-5c1d35447efc
using Colors

# ╔═╡ 5364eeb8-51e7-4d30-af37-ad7f62994978
mapconf = String(HTTP.get("https://rest.kegg.jp/get/map00010/conf").body)

# ╔═╡ b2d4efb8-c1eb-4ee7-91b9-e0a72f6f2ec9
mapdata = map(split(mapconf, "\n")) do line
	Dict(zip(["node","url","info"], split(line, "\t"; limit=3)))
end

# ╔═╡ ca2261ea-e070-4e66-bc37-844f87dd0b86
begin
	f1 = Figure()
	f1a1 = Axis(f1[1,1])
	...
	for entry in mapdata
		@show entry
		node = entry["node"]
		shape, loc1, loc2 = String.(split(node, " "))
	
		if shape == "rect"
			xmin, ymin = parse.(Int, split(loc1[begin+1:end-1], ","))
			xmax, ymax = parse.(Int, split(loc2[begin+1:end-1], ","))
	
			CairoMakie.poly!(f1a1,
				[
					(-xmin, -ymin),
					(-xmax, -ymin),
					(-xmax, -ymax),
		...
		elseif shape =="circ"
			CairoMakie.scatter!(f1a1,
				-1 .* Tuple(parse.(Int, split(loc1[begin+1:end-1], ","))),
				markersize=2*parse(Int64, loc2),
				color=:gainsboro,
			)
		end
	end
...
# ╔═╡ bcbeb75a-2fe8-492b-b3ff-03a617ca0c3f
begin
	url = "https://rest.kegg.jp/link/rn/map00010"
	response = HTTP.get(url)
	reactions = [split(line, "\t")[2][4:end] for line in split(String(response.body), "\n") if !isempty(line)]
end

# ╔═╡ 72cae8f3-0501-4a22-962b-9b6d102e1a4e
begin
...
# ╔═╡ a8cad9b6-ce1e-4c10-a16e-cf830820b77f
reaction_dict

# ╔═╡ 729ff95b-5998-441a-8410-02d844c4d031
mapcpd = begin
...
# ╔═╡ 8c2dd194-7990-4aeb-9006-4b9359e1962e
begin
	nodemapping = Dict()
	...
	for (rn,eq) in reaction_dict
		...
		if length(rn_cpd) > 0
			...
			for cpd in rn_cpd
				if !haskey(nodemapping, cpd)
					add_vertex!(H)
					nodemapping[cpd] = nv(H)
				end
	
				#@show "$(nodemapping[rn]) <=> $(nodemapping[cpd])"
				add_edge!(H, nodemapping[rn], nodemapping[cpd])
			end
			#println()
		end
	end
...
# ╔═╡ ff6a90da-6455-4668-8b63-0cdcefdcc040
H

# ╔═╡ 27f726f6-266e-43fb-b6ef-0008116b90fc
begin
	fixed_positions = map(mapdata) do entry
		node = entry["node"]
		shape, loc1, loc2 = String.(split(node, " "))
	
		if shape =="circ"
			cpdid = entry["info"][begin:begin+5]
			xy = -1 .* Tuple(parse.(Int, split(loc1[begin+1:end-1], ",")))
			if haskey(nodemapping, cpdid)
				Dict(nodemapping[cpdid] => xy)
			end
		end
	end |> x -> filter(!isnothing, x) |> x->reduce(merge,x)

	xmax = maximum(first.(collect(values(fixed_positions))))
	ymax = maximum(last.(collect(values(fixed_positions))))
	
	fixed_positions = Dict(
		i => Point2f(xy[1]/xmax, xy[2]/ymax)
		for (i, xy) in fixed_positions
	)
	pin = Dict(
		i => haskey(fixed_positions, i)
		for i in 1:nv(H)
	)
end

# ╔═╡ fcdac1c6-ce70-416c-86b1-2beb17473261
begin
	json_data = JSON3.read(open("../data/complexities_map00010.json", "r"))
...
# ╔═╡ 7469a8a0-5851-4834-b62a-dba9880fa42f
begin
	# Preparation
	idx2node = Dict(v=>k for (k,v) in nodemapping)
	...
	gp = graphplot!(f2a1,
		...
		node_color = [
			startswith(idx2node[i],"C") ? 
			begin
				ma = get(node2ma, idx2node[i], missing)
				if ismissing(ma)
					:gainsboro
				else
					cgrad(:Spectral)[(ma-minimum(values(node2ma)))/maximum(values(node2ma))]
				end
			end :
			:transparent
			for i in 1:nv(H)
	...
	Colorbar(
		f2[1, 2],
		limits = extrema(values(node2ma)),
		colormap = :Spectral,
    	label = "MA",
		ticks = [extrema(values(node2ma))...; middle(extrema(values(node2ma))...)]
	)
	f2
end
...
manifest_format = "2.0"
project_hash = "04149139318203bf96a18fa7695c4677b547c892"
